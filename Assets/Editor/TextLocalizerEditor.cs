...
public class TextLocalizerSearchWindow : EditorWindow
{
    public static void Open()
    {
        ...
        window.titleContent = new GUIContent("Localization Searcher");
    ...
    private void GetSearchResults()
    {
        ...
        foreach(KeyValuePair<string,string> element in dictionary)
        {
            if (element.Key.ToLower().Contains(value.ToLower()) || element.Value.ToLower().Contains(value.ToLower()))
            {
                ...
                {
                    if (EditorUtility.DisplayDialog("Remove Key" + element.Key + "?", "This will ruin your life, deleting this forever...","Do it, please."))
                    {
